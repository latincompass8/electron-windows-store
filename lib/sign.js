...
function _sign (program, ext) {
  return new Promise((resolve, reject) => {
    if (!program.devCert) {
      utils.debug(`Error: Tried to call _sign, but program.devCert was undefined`)
      return reject(new Error('No developer certificate specified!'))
    }

    const pfxFile = program.devCert
    const appxFile = path.join(program.outputDirectory, `${program.packageName}.${ext}`)
    ...
    utils.debug(`Signing ${ext} package: ${appxFile}`)
...
function signAppx (program) {
     return _sign(program, 'appx');
}

function signMSIX (program) {
     return _sign(program, 'msix');
}
